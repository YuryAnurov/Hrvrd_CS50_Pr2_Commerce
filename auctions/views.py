...
def index(request):
    return render(request, "auctions/index.html", {"listings": Listing.objects.all(), "wtch": wtchlisted(request)})


def categories(request):
    return render(request, "auctions/categories.html",
                  {"categories": Category.objects.all(), "wtch": wtchlisted(request)})


def catlistings(request, catname):
    return render(
        request, "auctions/catindex.html", {"listings": Listing.objects.filter(category__catname=catname),
                                            "wtch": wtchlisted(request), 'catname': catname})
...
#    return listing(request, listing_id) - здесь меня обругали, говорят Listing not callable
    ...
    #               {"listing": listing, "comments": Comment.objects.filter(listing=listing), "addwatch": True})
...
def addwatch(request):
    if request.method == "POST":
        ...
        f = request.user
        if listing in f.watchlist.all():  # не смог проголосовать из-за 0 репутации на stackoverflow за хороший ответ
...
#         addwatch = True
#     return render(request, "auctions/listing.html", {
#         "listing": listing, "comments": Comment.objects.filter(listing=listing), "addwatch":addwatch,
#         "wtch": wtchlisted(request)})
...
def listing(request, listing_id):
            ...
            if madebid <= curbd:
                ...
                return render(
                    ...
                    {"listing": listing, "comments": Comment.objects.filter(listing=listing),
                     "addwatch": addwatch, "wtch": wtchlisted(request), "message": mssg})
        elif madebid < listing.starting_bid:
    ...
    return render(request, "auctions/listing.html", {
        "listing": listing, "comments": Comment.objects.filter(listing=listing),
        "addwatch": addwatch, "wtch": wtchlisted(request)})
